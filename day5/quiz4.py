...
cont = data.groupby("continent")["spirit_servings"].agg(["mean","min","max","sum"])
index = cont_mean.index
bar_width = 0.25
plt.bar(index, cont_mean, color='red')
plt.bar(index+bar_width, cont_min, color='green')
plt.bar(index+2*bar_width, cont_max)
plt.bar(index+3*bar_width, cont_sum)
plt.show()
